...
export const setBoards = (currentUserId) => async (dispatch) => {
    try {
        ...
        // Test code for manually inserted data in DB. users collection based on the custom 'id' field
        const userQuery = query(usersCollection, where("id", "==", currentUserId));
        ...
        // Query boards using the fetched board id
        const q = query(boardsCollection, where("id", "in", userBoardIds));
        const snapshot = await getDocs(q);

        // console.log("Snapshot docs:", snapshot.docs.map(doc => doc.data()));
        
        // for the real data code(ID generated by firebase)
        //const boards = snapshot.docs.map((doc) => ({ id: doc.id, ...doc.data() }));
        const boards = snapshot.docs.map((doc) => ({ id: doc.data().id, ...doc.data() }));
...
export const searchBoard = (boardId) => async (dispatch) => {
    try {
        const q = query(boardsCollection, where("id", "==", Number(boardId)));
        const snapshot = await getDocs(q);
